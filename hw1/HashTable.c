...
// Takes in a chain in the HashTable, a target key, and a pointer to the
// iterator and finds target key node in a given chain. Edits the iterator
// pointer to have iter point to the specified target. Returns bool for
// whether or not target was found
...
bool HashTable_Insert(HashTable *table,
  ...
  LLIterator *iter;
  HTKey_t t_key = newkeyvalue.key;

  HTKeyValue_t *copy = (HTKeyValue_t*) malloc(sizeof(HTKeyValue_t));
  Verify333(copy != NULL);

  copy -> key = newkeyvalue.key;
  copy -> value = newkeyvalue.value;

  // If key already exists, return old value and overwrite it
  if (FindInChain(chain, t_key, &iter)) {
    LLPayload_t payload;
    LLIterator_Get(iter, &payload);
    HTKeyValue_t *kv = (HTKeyValue_t*)payload;

    // Populate oldkeyvalue
    oldkeyvalue -> key = kv -> key;
    oldkeyvalue -> value = kv -> value;

    // Overwrite stored entry with new one
    kv -> key = copy -> key;
    kv -> value = copy -> value;
...
bool HTIterator_Next(HTIterator *iter) {
  ...
  // If current bucket doesn't have any more nodes, search for next
  // bucket with more nodes and create iterator for that one
  if (!LLIterator_Next(iter -> bucket_it)) {
    ...
    while ((iter -> bucket_idx) < (iter -> ht -> num_buckets)) {
      ...
      if (LinkedList_NumElements(iter -> ht -> buckets[idx]) > 0) {
    ...
    // No node left, iter should be null and idx should be invalid
    iter -> bucket_it = NULL;
    iter -> bucket_idx = INVALID_IDX;
    return false;
  }
...
static bool FindInChain(LinkedList *chain, HTKey_t t_key,
                        LLIterator **new_iter) {
