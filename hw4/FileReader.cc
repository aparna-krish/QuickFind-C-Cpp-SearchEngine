...
bool FileReader::ReadFile(string *const contents) {
  ...
  buffer = ReadFileToString(full_file.c_str(), &f_size);
  if (! buffer) {
    cerr << "Error reading file: " << full_file << endl;
    return false;
  }

  *contents = string(buffer, f_size);
  // Free buffer, since under the hood dynamically allocated
  free(buffer);
  return true;
}
