...
int WriteIndex(MemIndex *mi, DocTable *dt, const char *file_name) {
  ...
  // If failed on writing MemIndex, close file
...
static int WriteHeader(FILE *f, int doctable_bytes, int memidx_bytes) {
  ...
  // Move file ptr to position right after header
  fseek(f, sizeof(IndexFileHeader), SEEK_SET);

  u_int8_t buffer[4096];
  size_t bytes_read;
  size_t to_read = doctable_bytes + memidx_bytes;

  // Read file in chunks until everything is read
  while (to_read > 0 &&
    (bytes_read = fread(buffer, 1, sizeof(buffer), f)) > 0) {
    // For each byte update CRC32 checksum
    for (size_t i = 0; i < bytes_read; i++) {
      crc.FoldByteIntoCRC(buffer[i]);
    }
    // Decrement count of bytes to read by processed bytes
    to_read -= bytes_read;
  }
...
static int WriteHashTable(FILE *f, IndexFileOffset_t offset, HashTable *ht,
  ...
  for (int i = 0; i < ht->num_buckets; i++) {
    // STEP 4.
    // Get num elements in curr bucket's list
    int32_t chain_len = (int32_t) (LinkedList_NumElements(ht -> buckets[i]));

    // Write HT Bucket metadata record to file
    if (WriteHTBucketRecord(f, record_pos, chain_len, bucket_pos) < 0) {
      return kFailedWrite;
    }

    // Increment file position by size of BucketRecord
    record_pos += sizeof(BucketRecord);

    // Write data in bucket to file
    int bucket_increment =  WriteHTBucket(f, bucket_pos, ht->buckets[i], fn);
    if (bucket_increment < 0) {
      return kFailedWrite;
    }

    // Increment bucket position
...
static int WriteHTBucket(FILE *f, IndexFileOffset_t offset, LinkedList *li,
  ...
  for (int i = 0; i < num_elts; i++) {
    ...
    // If seek fails, clean up iterator and return failure
    if (fseek(f, record_pos, SEEK_SET) != 0) {
      LLIterator_Free(it);
      return kFailedWrite;
    }

    // Create ElementPositionRecord object and convert fiels to network byte order
    ElementPositionRecord el_rec(element_pos);
    el_rec.ToDiskFormat();

    // Write ElementPositionRecord to file at correct position
    // If fwrite fails, clean up and return failure
    if (fwrite(&el_rec, sizeof(ElementPositionRecord), 1, f) != 1) {
      LLIterator_Free(it);
      return kFailedWrite;
    }
...
static int WriteDocidToDocnameFn(FILE *f, IndexFileOffset_t offset,
  ...
  const char *filename = (const char*)(kv -> value);
  ...
  if (fwrite(filename, sizeof(char), file_name_bytes, f) != (size_t)(file_name_bytes)) {
    return kFailedWrite;
  }
...
static int WriteDocIDToPositionListFn(FILE *f,
  ...
  for (int i = 0; i < num_positions; i++) {
    ...
    DocPositionOffset_t truncate = (DocPositionOffset_t)((uintptr_t)(payload) & 0xFFFFFFFF);

    // Create DocIDElementPosition object with truncated value
  ...
  return num_positions*sizeof(DocIDElementPosition) + sizeof(DocIDElementHeader);
}
...
static int WriteWordToPostingsFn(FILE *f,
  ...
  int16_t word_bytes = (int16_t)(strlen(wp -> word));
  ...
  // Construct the WordPostingsHeader object
  WordPostingsHeader header(word_bytes, ht_bytes);
  header.ToDiskFormat();

  // Seek to specified offset where header should be
  if (fseek(f, offset, SEEK_SET) != 0) {
    return kFailedWrite;
  }

  // Write WordPostingsHeader to disk at offset
