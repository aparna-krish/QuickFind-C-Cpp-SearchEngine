...
bool DocIDTableReader::LookupDocID(
  ...
  for (IndexFileOffset_t &curr : elements) {
    ...
    if (curr_header.doc_id == doc_id) {
      ...
      DocIDElementPosition elem_pos;
      for (int i = 0; i < curr_header.num_positions; i++) {
        Verify333(fread(&elem_pos, sizeof(DocIDElementPosition), 1, file_) == 1);
        elem_pos.ToHostFormat();
        ret_val -> push_back(elem_pos.position);
      }
...
list<DocIDElementHeader> DocIDTableReader::GetDocIDList() const {
  ...
  for (int i = 0; i < header_.num_buckets; i++) {
    ...
    // the index file.
    Verify333(fseek(file_, offset_ + sizeof(BucketListHeader) + i*sizeof(BucketRecord), SEEK_SET) == 0);
    ...
    for (int j = 0; j < bucket_rec.chain_num_elements; j++) {
      ...
      Verify333(
        ...
        // Seek file pointer to element's actual position
