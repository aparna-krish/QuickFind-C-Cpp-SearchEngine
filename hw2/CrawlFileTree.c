...
static void HandleDir(char *dirpath, DIR *d, DocTable **doctable,
  ...
  for (i = 0;; i++) {
    /* Read the next entry; break on end-of-directory */
    // STEP 2.
    ...
    if (stat(entries[i].path_name, &st) == 0) {
      ...
      if (stat(entries[i].path_name, &st) == 0) {
        if (S_ISREG(st.st_mode)) {
          ...
          /* neither file nor directory: drop this entry */
          free(entries[i].path_name);
          i--;
          continue;
        }
      } else {
        /* stat failed: drop this entry */
        free(entries[i].path_name);
        i--;
        continue;
      }
...
static void HandleFile(char *file_path, DocTable **doctable,
  ...
  // Add file to doc table and get doc_id
  doc_id = DocTable_Add(*doctable, file_path);
  // Allocate iterator to walk through hash table
  it = HTIterator_Allocate(tab);
  Verify333(it != NULL);

  // Loop over each word's position list in hash table
