...
void MemIndex_AddPostingList(MemIndex *index, char *word, DocID_t doc_id,
  ...
  if (!HashTable_Find(index, key, &mi_kv)) {
    ...
    // Allocate WordPostings struct
    wp = (WordPostings*) malloc(sizeof(WordPostings));
    Verify333(wp != NULL);

    wp -> word = word;

    // Create hash table to map DocID to positions list
    HashTable *wp_HT = HashTable_Allocate(HASHTABLE_INITIAL_NUM_BUCKETS);
    wp->postings = wp_HT;

    // Prepare key-value pair for insertion
    mi_kv.key = key;
    mi_kv.value = (HTValue_t) wp;

    // Insert new postings table into inverted idx
...
LinkedList* MemIndex_Search(MemIndex *index, char *query[], int query_len) {
  ...
  // Hash first query term into lookup key
  key = FNVHash64((unsigned char*) query[0], strlen(query[0]));

  // If word isn't in the index then no matches exist
  if (!HashTable_Find(index, key, &kv)) {
    return NULL;
  }

  // Extract pointer from hash table result
  wp = kv.value;

  // Allocate iterator to walk postings list for this word
  HTIterator *iter = HTIterator_Allocate(wp -> postings);
  Verify333(iter != NULL);

  // Create LinkedList to hold SearchResults
  ret_list = LinkedList_Allocate();
  Verify333(ret_list != NULL);

  // For each posting in list
  while (HTIterator_IsValid(iter)) {
    // Allocate new Searchresult structure
    SearchResult *sr = (SearchResult*) malloc(sizeof(SearchResult));
    Verify333(sr != NULL);

    HTIterator_Get(iter, &kv);
    // Key is in the DocID and value is positions list
    sr -> doc_id = (DocID_t) kv.key;
    sr -> rank = LinkedList_NumElements((LinkedList*) kv.value);

    // Append this SearchResult to result list
  ...
  for (i = 1; i < query_len; i++) {
    ...
    for (j = 0; j < num_docs; j++) {
      // Get SearchResult pointer
      SearchResult *sr;
      LLIterator_Get(ll_it, (LLPayload_t*) &sr);

      // If doc has curr word add occurrence count to sr -> rank
      if (HashTable_Find(wp->postings, (HTKey_t) sr->doc_id, &kv)) {
        sr->rank += LinkedList_NumElements((LinkedList*) kv.value);
        LLIterator_Next(ll_it);
      } else {
        // Delete this SearchResult from ret_list
